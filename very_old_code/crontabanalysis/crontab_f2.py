...
class CrontabAnalysis:
    ...
    def __init__(self, cFile):
        ...
        self.ptDate1 = re.compile(r'''
           ...
           ''', re.VERBOSE)
        part = r'(((\d{1,2}-\d{1,2}|\*)(/\d{1,2})?|\d{1,2})(,\s*)?)+'
        self.ptDate = re.compile(r'\s*(?P<minute>' + part
                                + r')\s+(?P<hour>' + part
                                + r')\s+(?P<day>' + part
                                + r')\s+(?P<month>' + part
                                + r')\s+(?P<week>' + part
    ...
    def getTime1(self, sometime, rest):
        ...
        #elif abc = ptTemp3.search(sometime):
        elif ',' in sometime:                       # case: a,b,
            timeString = '在'
            tl = sometime.split(',')
            for i in tl:
                timeString += self.getTime(i, rest) + '￥￥'
                #timeString += self.getString(i.strip(), rest) + ','
            timeString = timeString[:-1]
            #timeString += rest
            return timeString
        else:                                       # case: n
            #return '第' + sometime + rest
