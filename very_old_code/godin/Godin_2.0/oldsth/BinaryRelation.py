...
class FormalContext(object):
    ...
    def random_br(self, object_num=0, attribute_num=0, object_attribute_rate=1):
        ...
        # 生成随机对象，属性值
        self.attribute_list = ['a%d' % att for att in range(attribute_num)]
        self.object_list = ['o%d' % obj for obj in range(object_num)]
    ...
    def __str__(self):
        """输出形式背景信息
        """
        return 'object_num is %s\nobject_list is %s\natribute_num is %s\nattribute_list is %s\nbr_list is\n%s\noa_value is\n%s' % (self.object_num, self.object_list, self.attribute_num, self.attribute_list, self.br_list, self.oa_value)
...
def read_br_file(filename='formalcontext'):
...
if __name__ == "__main__":
    ...
    #fc.random_br()
    #print fc
    #fc.random_br(10, 10, 0.3)
    #print fc
    #fc.add_object_list([1,2,3])
    #fc.add_attribute_list(['a', 'b', 'c'])
    #fc.add_br_list([1])
    #print fc
    #save_FC(fc)
    print read_br_file('binary_relation')