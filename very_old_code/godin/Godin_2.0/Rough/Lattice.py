...
class Lattice(object):
    ...
    some...
    """
    def __init__(self, obj, att, b1, b2):
        """初始化
    ...
    def get_power(self, A, B):
        """给出幂集
        """
        un = A.union(B)
        for one in Permutation(list(un)).perm_all():
            yield one
    
    def get_Hn_Concept(self, A, B, intSet, fxintSet):
        """增加精度系数，b1, b2
        """
        for mex in self.get_power(A, B):
            tempmex = list(mex)
            mex = set(mex)
            mexs = self.o2a[tempmex.pop()]
            for i in tempmex:
                mexs = mexs.intersection(self.o2a[i])
                if not mexs:
                    break
            if not mexs:
                continue
    ...
    def delete_cpt(self, cpt):
        """删除某个概念，调整边即父子关系
        """
        for p in cpt.parent_list:
            self.deleteEdge(p, cpt)
        for c in cpt.child_list:
            self.deleteEdge(cpt, c)
        for p in cpt.parent_list:
            for c in cpt.child_list:
                self.addEdge(p, c)
    ...
    def addOneObject(self, oneObject, oneattribute):
        ...
        self.o2a[oneObject[0]] = set(oneattribute)
        for a in oneattribute:
            if self.a2o.has_key(a):
                self.a2o[a].add(oneObject[0])
            else:
                self.a2o[a] = set(oneObject)
        ...
        for i, H_list in sorted(C.items(), key=lambda x:x[0]):# 从intent的数目由小到大考虑各个概念
            for H in H_list:
                ...
                if H.intent == fxSet:                                         # 若这个概念的intent＝fxSet，exit algorithm
                    ...
                    for intSet in self.get_power(H.intent, fxSet): # 修改了这里，增大了intSet
                        tmpintSet = list(intSet)
                        intSet = set(intSet)
                        fxintSet = self.a2o[tmpintSet.pop()]
                        for i in tmpintSet:
                            fxintSet = self.a2o[i].intersection(fxintSet)
                            if not fxintSet:
                                break
                        if not fxintSet:
                            continue
                        ...
                        if not flaglist:                                                   # not exisit, show that H is a generator
                            ...
                            for Hn in self.get_Hn_Concept(H.extent, xSet, intSet, fxintSet):
                                # 删掉属性冗余概念和对象冗余概念
                                        ...
                                        if Ha.intent <= intSet:                      # Ha is a potential parent of Hn 
                                            ...
                                            for Hd in Ha.child_list:
                                                if Hd.intent <= intSet:              # 只要有一个更新概念的孩子的intent包含于这个交集，就不是父节点
                                                    ...
                                                    break
                                            if parent:                                      # 是父节点
                                                if Ha in H.parent_list:               # 当前的概念是不是他的孩子，是的话，删除边
