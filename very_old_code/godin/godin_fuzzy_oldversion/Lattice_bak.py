...
class Lattice:
	...
	def addOneObject(self, oneObject, oneattribute):
		...
		fxSet = set(oneattribute)	   					#the set of oneobject's intent
...
class Concept:
	...
	def __str__(self):
		...
		return 'Concept %d: (%s), (%s)\n%s\nxigema is %d and lanmena is %d' % (self.conceptID, self.extent, self.intent, str_child[:-2], self.xigema, self.lanmena)
