...
class Lattice:
	...
	def addEdge(self, parent_cpt, child_cpt):
		...
		#newedge = Edge(parent_cpt.conceptID, child_cpt.conceptID)
	...
	def deleteEdge(self, parent_cpt, child_cpt):
		...
		#	if oneedge.getEdge() == (parent_cpt.conceptID, child_cpt.conceptID):
	...
	def SelectAndClassifyPairs(self, fxSet):
		...
		key_list.sort()
		if len(self.layer[key_list[-1]]) == 1: 
		    lay = key_list[-2]
		else:
		    lay = key_list[-1]
		for x in fxSet:
	...
	def addOneObject(self, oneObject, oneattribute):
		...
		if not self.supConcept.extent and not self.supConcept.intent:
			...
			###self.addEdge(self.botConcept, self.supConcept)#new
			return True
		...
		C_list = C.keys()
		C_list.sort()
		for i in C_list:#new							# 从intent的数目由小到大考虑各个概念
			for H in C[i]:
				if H.intent <= fxSet:
