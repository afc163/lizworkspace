    ...
    for onefile in file_term:
        ...
        for oneterm in tdf:
            if oneterm in file_term[onefile]:  # 如果该文档有term, 则计算对应权值, 否则为0
                cf = CF[oneterm][file_category[onefile][0]]
                if cf:
                    dvs_table[onefile][oneterm] = TF[oneterm][onefile]*log(1.0/DF[oneterm]+DELTA)*log(1.0/cf+DELTA)
                else:
                    dvs_table[onefile][oneterm] = TF[oneterm][onefile]*log(1.0/DF[oneterm]+DELTA)
                    print 'cf is zero'
...
def classify():
    ...
    for (testfile, category, tf) in get_test_file():
        ...
        testfile_test_category[testfile] = [sorted(category_count.items(), key=lambda c:c[1])[-1][0]]
        print testfile_test_category[testfile]
        
    pickle.dump(testfile_true_category, open('testfile_true_category.dump', 'w'))
    pickle.dump(testfile_test_category, open('testfile_test_category.dump', 'w'))

def compute_r_p():
    """计算准确率和召回率
    """
    true_category = pickle.load(open('testfile_true_category.dump'))
    test_category = pickle.load(open('testfile_test_category.dump'))
    category1 = collections.defaultdict(lambda: [])
    category2 = collections.defaultdict(lambda: [])
    
    for onefile in true_category:
        category1[true_category[onefile][0]].append(onefile)
    for onefile in test_category:
        category2[test_category[onefile][0]].append(onefile)
    
    for ct in category1:
        print ct                  ## 这里为了便于画图, 保存到本地文件.
        A = set(category1[ct])
        B = set(category2[ct])
        a = len(A.intersection(B)) # 正确分到对应类别的文档个数
        b = len(B-A)# 被误分到该类的文档个数
        c = len(A-B) #属于该类但误分到其他类别的文档个数
        print a, b, c
        try:
            r = a*1.0/(a+c)
            p = a*1.0/(a+b)
    ...
    for onefile in true_category:
        total += 1
        if true_category[onefile][0] == test_category[onefile]:
            category[test_category[onefile]] += 1
            a += 1
        else:
            error += 1
    print category
    print 'error: %s in total: %s' % (error, total)
    """

def average_item_count():
    """
    to 查看特征空间中每篇文档的特征数平均
    """
    file_term = pickle.load(open('file_term.dump'))
    summ = 0
    for onefile in file_term:
        summ += len(file_term[onefile])
    print summ*1.0/len(file_term)
...
def read_r_p_f(filename):
    ...
    for i in open(filename).readlines():
        if i.startswith('r: '):
            r.append(float(i[3:-1]))
        elif i.startswith('p: '):
            p.append(float(i[3:-1]))
        elif i.startswith('F: '):
            f.append(float(i[3:-1]))
    return (r, p, f)
    
def generate_chart():
    """生成图表
    """
    def _generate_script_string(rpf1, rpf2):
        """生成数据字符串
        """
        table_string = "data.addColumn('string', '类别');\ndata.addColumn('number', 'TF-IDF');\ndata.addColumn('number', 'TF-IDF-CF');\n"
        colnum = len(category)
        rownum = 3
        table_string += "data.addRows(%s);\n" % colnum
        for i in range(colnum):
            for j in range(rownum):
                if j == 0:
                    table_string += "data.setValue(%s, %s, '%s');\n" % (i, j, category[i])
                elif j == 1:
                    table_string += "data.setValue(%s, %s, %s);\n" % (i, j, rpf1[i])
                elif j == 2:
                    table_string += "data.setValue(%s, %s, %s);\n" % (i, j, rpf2[i])
    ...
    def _generate_html(valuestring, title):
        """生成整个html文档
        """
        return '''
<html>
...
%s
        ...
        chart.draw(data, {width: 1000, height: 500, legend: 'bottom', titleY: '%s'});
      }
    </script>
  </head>
  <body>
    ...
    try:
        (tfidf_r, tfidf_p, tfidf_f) = read_r_p_f('result/tfidf.rpf')
        (tfidfcf_r, tfidfcf_p, tfidfcf_f) = read_r_p_f('result/tfidfcf.rpf')
        (tfidficf_r, tfidficf_p, tfidficf_f) = read_r_p_f('result/tfidficf.rpf')
    except:
        print 'read rpf error'
        return
    
    category = []
    for c in open('result/category').readlines():
        category.append(c.strip())
    
    open('r.html', 'w').write(_generate_html(_generate_script_string(tfidf_r, tfidfcf_r), '召回率(r)'))
    open('p.html', 'w').write(_generate_html(_generate_script_string(tfidf_p, tfidfcf_p), '准确率(p)'))
    open('f.html', 'w').write(_generate_html(_generate_script_string(tfidf_f, tfidfcf_f), 'F-指标'))
    
    #open('r1.html', 'w').write(_generate_html(_generate_script_string(tfidf_r, tfidficf_r), '召回率(r)'))
    #open('p1.html', 'w').write(_generate_html(_generate_script_string(tfidf_p, tfidficf_p), '准确率(p)'))
    #open('f1.html', 'w').write(_generate_html(_generate_script_string(tfidf_f, tfidficf_f), 'F-指标'))
