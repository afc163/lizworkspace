...
class FormalContext(object):
    ...
    def add_one_object_att(self, obj, att):
        """增加一个对象属性集
        """
        try:
            idx = self.object_list.index(obj)
            self.oa_value[idx] = att
        except:
            pass
    ...
    def __str__(self):
        """输出形式背景信息
        """
        return 'object_num is %s\nobject_list is %s\natribute_num is %s\nattribute_list is %s\nbr_list is\n%s\noa_value is\n%s' % (self.object_num, self.object_list[:10], self.attribute_num, self.attribute_list[:10], self.br_list, len(self.oa_value))
