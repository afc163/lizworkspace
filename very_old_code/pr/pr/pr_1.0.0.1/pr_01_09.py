...
class RelationFiles(object):
    ...
    def cd_handle(self, line):
        ...
        #如果cd 后面跟的是变量名，即路径存放在变量中，搜索策略
            current_dir=self.doll_handle(dir[1:])
        else:
    ...
    def find_relation_other(self):
        ...
        for line in self.file_content:
            ...
            #    self.add_child(self.get_abspath(line)[:-1])
            #elif line.startswith('sh ') or line.startswith('perl ') or line.startswith('python '):
            #    self.sh_perl_python_handle(line)###或者是不要这样，在一行中提取关键字??
            #else:###??aa.py aa.sh aa.pl aa
            #    self.other_handle(line)
            pt_file = re.compile(r'(?P<file>[$.~/a-zA-Z0-9_-]*\.(sh|py|pl|txt|dict))\b')
            teams = pt_file.findall(line)
            for (one, onetype) in teams:
                self.add_child(self.get_abspath(one)[:-1])
                
    def find_relation(self):#X
        """
        find_relation寻找当前file调用的程序、脚本、配置等等文件
        """
        for line in self.file_content:
            ...
            if line.startswith('./'):
                ...
                self.slash_handle(line)
            elif line.startswith('~/'):
                self.wave_slash_handle(line)
            elif line.startswith('sh ') or line.startswith('perl ') or line.startswith('python '):
                self.sh_perl_python_handle(line)###或者是不要这样，在一行中提取关键字??
            else:#####??aa.py aa.sh aa.pl aa
                self.other_handle(line)
    def add_child(self, file):
        ...
        #    if self.child_files.has_key('sh'):###类型可以用数字表示，或用枚举？
        #        self.child_files['sh'].append(file)
        #    else:
        #        self.child_files['sh'] = [file]
        #elif filename.endswith(".pl"):
        #    if self.child_files.has_key('pl'):###类型可以用数字表示，或用枚举？
        #        self.child_files['pl'].append(file)
        #    else:
        #        self.child_files['pl'] = [file]
        #elif filename.endswith(".py"):
        #    if self.child_files.has_key('pl'):###类型可以用数字表示，或用枚举？
        #        self.child_files['py'].append(file)
        #    else:
        #        self.child_files['py'] = [file]
        #elif filename.endswith(".conf"):
        #    if self.child_files.has_key('conf'):###类型可以用数字表示，或用枚举？
        #        self.child_files['conf'].append(file)
        #    else:
        #        self.child_files['conf'] = [file]
        #elif filename.endswith(".txt"):
        #    if self.child_files.has_key('txt'):###类型可以用数字表示，或用枚举？
        #        self.child_files['txt'].append(file)
        #    else:
        #        self.child_files['txt'] = [file]
        #elif filename.endswith(".dict"):
        #    if self.child_files.has_key('dict'):###类型可以用数字表示，或用枚举？
        #        self.child_files['dict'].append(file)
        #    else:
        #        self.child_files['dict'] = [file]
        #else:
        #    if self.child_files.has_key('c'):###其他都认为是c程序
        #        self.child_files['c'].append(file)
        #    else:
        #        self.child_files['c'] = [file]
    ...
    def slash_handle(self, line):#X
        """
        /file.sh /file.py /file.pl
        """
        teams=line.split(' ')
        child_pro=teams[0].strip()
        self.add_child(child_pro)
        
    def dot_slash_handle(self, line):#X
        """
        ./file.sh ./file.py ./file.pl
        """
        teams=line.split(' ')
        #child_pro用于记录当前脚本搜索到的子程序或者脚本
        child_pro=teams[0].strip().replace('./',self.current_dir[-1])
        self.add_child(child_pro)###类型？？？？
    def wave_slash_handle(self, line):#X
        ...
        teams = line.split(' ')
        #child_pro用于记录当前脚本搜索到的子程序或者脚本
        child_pro = teams[0].strip().replace('~', '/home/'+os.getlogin())
        self.add_child(child_pro)
    
    def sh_perl_python_handle(self, line):#X
        """
        sh/perl/python file.sh file.py file.pl
        """
        pt_sh = re.compile(r'\s+(?P<name>[.~/a-zA-Z0-9_-]*\.(sh|py|pl))\b')
        teams = pt_sh.findall(line)
        for (one, onetype) in teams:
            if one.startswith('./') or one.startswith('/') or one.startswith('~/') or one.startswith('../'):
                self.add_child(self.get_abspath(one)[:-1])            
            else:
                #self.other_handle(one)
                child_pro = self.current_dir[-1] + one
                self.add_child(child_pro)
    
    def other_handle(self, line):#X
        """
        file.sh file.py file.pl
        """
        pt_sh = re.compile(r'(?P<name>[.~/a-zA-Z0-9_-]*\.(sh|py|pl))\b')
        teams = pt_sh.findall(line)
        for (one, onetype) in teams:
            #child_pro用于记录当前脚本搜索到的子程序或者脚本
            child_pro = self.current_dir[-1] + one
...
def file_handle(file):
    ...
    if not fr_graph.isHandled(file):             ###是否已经处理了
        try:
            ...
            file_relation.find_relation_other()       ###分析shell脚本
