...
class RelationFiles(object):
        ...
        if value.startswith('`'):
            ...
            for i in items:
                ...
                if os.path.isfile(file_path):
                    self.dot_slash_handle(file_path+value[value.find(i)+len(i):]) ##case a=`python pr.py -d mp3`
                    return
        if value.startswith('$('):
            ...
            for i in items:
                file_path = self.get_abspath(i)[:-1]
                if os.path.isfile(file_path):
                    self.dot_slash_handle(file_path+value[value.find(i)+len(i):])
                    return 
    ...
    def dot_slash_handle(self, line, type=None):
        ...
        if (type is None or 'BASH' in type) and child_pro.endswith('.sh'):
            self.add_child(child_pro, type='BASH', args=args_list)            
        elif (type is None or 'PYTHON' in type) and child_pro.endswith('.py'):
            self.add_child(child_pro, type='PYTHON')
        elif (type is None or 'PERL' in type) and child_pro.endswith('.pl'):
            self.add_child(child_pro, type='PERL')
        elif (type is None or 'CONF' in type) and child_pro.endswith('.conf'):
            self.add_child(child_pro, type='CONF')
        elif (type is None or 'C' in type) and child_pro.split('/')[-1].find('.') == -1:
            #C
            self.add_child(child_pro, type='C', args=args_list)
        else:
            #other type
            pass
    ...
    def sh_handle(self, line):
        ...
        #        break
        for i in range(len(teams)):
            dot_index = teams[i].find('.')  #find first .sh or .py or .pl
            if dot_index != -1:
                file_type = teams[i][dot_index:]
                args_list = []
                for a in teams[i+1:]:#get args... ignor -o -i
                    if a.startswith('-'):
                        pass
                        ...
                        args_list.append(self.get_abspath(a.strip())[:-1])   
                self.add_child(self.get_abspath(teams[i])[:-1], type=FILETYPE[file_type], args=args_list)
                break
                
    def perl_handle(self, line):#X?
        """
        perl xxx file.pl xxxx
        """
    ...
    def find_relation(self, type=None):
        ...
        for line in self.file_content:
            ...
            if line.startswith('cd ') or line == "cd":
                self.cd_handle(line)
            elif (type is None or 'FTP/HTTP' in type) and line.startswith('wget '):#####
                self.wget_handle(line, type)
            elif line.startswith('./') or line.startswith('/') or line.startswith('../') or line.startswith('~/'):
                self.dot_slash_handle(line, type)
            elif (type is None or 'BASH' in type) and line.startswith('sh '):
                self.sh_handle(line)
            elif (type is None or 'PYTHON' in type) and line.startswith('python '):
                self.sh_handle(line)
            elif (type is None or 'PERL' in type) and line.startswith('perl '):
                self.sh_handle(line)
            elif (type is None or 'CONF' in type) and '.conf' in line:
                self.conf_handle(line)
            elif (type is None or 'DICT' in type) and '.dict' in line:######
                pass
            elif '=' in line:
