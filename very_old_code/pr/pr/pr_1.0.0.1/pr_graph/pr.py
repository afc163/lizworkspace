...
FILE_NUM = {'.sh':0, '.pl':1, '.py':2, '.c':3, '.conf':4, '.txt':5, '.dict':6}
class RelationFiles(Graph):
    ...
    def handle_current_file(self, file):
        try:
    ...
    def add_child(self, file):
        ...
        if dot_index != -1:
            ...
            type = '.c'
        type_num = FILE_NUM[type]
        self.add_node(file)
        self.add_edge(self.file, file, type)
...
def file_handle(file):
    ...
    if not fr_graph.isHandled(file):             ###是否已经处理了
        try:
            fr_graph.handle_current_file(file)
            #for child_files in fr_graph.get_current_children['.sh']:
