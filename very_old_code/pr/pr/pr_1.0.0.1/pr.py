...
def do_main(argv=None):
    ...
    if args_sign:
        ...
        if 'db' in answer:
            #save to prdb
            save_to_db(relation_files)
        if 'file' in answer:
            #save to prdb
            save_to_file('relations.dump', relation_files)

def file_to_db(filename):
    """change relations in file to db
    """
    import pickle
    try:
        r_f = pickle.load(open(filename))
        save_to_db(r_f)
...
def save_to_db(relation_files):
            ...
            mydb = None
            try:
                mydb = MyDBTool(host='localhost', db='prdb', user='root', passwd='lizzie1985')
                mydb.some_prepare()
