...
def try_download(h, pagenumber=1):
...
def download_search_page():
    ...
    try:
        ...
        h = httplib2.Http()
        total_page_num = analysis_page(try_download(h), True)
        if total_page_num:
            print 'total page ', total_page_num
        for pn in xrange(2, total_page_num+1):
           analysis_page(try_download(h, pn)) 
           time.sleep(1)
        # 保存RESULT
        pickle.dump(RESULT, open(RESULT_FILE, 'w'))

    print "\nObtain %s search results and now, downloading all\n" % len(RESULT)
    for (idd, name) in RESULT:
        download_item(idd, name)
        time.sleep(1)
