...
class Bloom (object):
  ...
  def __init__ (self, bytes, hashes, data = None):
    '''@bytes is the size of the bloom filter in 8-bit bytes and @hashes is the number of hash functions to use. Consult the web page linked above for values to use. If in doubt, bytes = num_elements and hashes = 4'''
    self.hashes = hashes
    self.bytes  = bytes
  ...
  def _make_array (self, size):
    ...
    todo = size # 为什么不直接array.array ('B', '\x00' * size)?估计这个size不是正巧是size,是得满足256,,,,,不过似乎不对阿,
    while (todo >= blocklen):
      a.extend (arrayblock)
      todo -= blocklen
    if todo:
  ...
  def _hashfunc (self, n, val):
    ...
    b = [ord(x) for x in struct.pack ('I', val)] #struct.pach是根据I将val格式化成一个字符串,返回的是一个字符串,然后b中存放的是字符串中对应字符的ascii编号
