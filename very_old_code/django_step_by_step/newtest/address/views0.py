...
///////////////
from djangp.views.generic.list_detail import object_list
def search(request):
	name = request.REQUEST['search']
	if name:
		#extra_lookup_kwargs = {'name__icontains':name}
		extra_context = {'searchvalue':name}
			 #return object_list(request, Address, paginate_by=10, extra_context=extra_context,  extra_lookup_kwargs=extra_lookup_kwargs)
