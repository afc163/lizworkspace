...
KISSY.add('starrating', function(S){
    var DOM = S.DOM,
        ...
        MA_RATING_CURRENT = '.current-rating',
        ...
        <div class="rating-bd" id="J_Rating">
            ...
            <div class="shop-rating">
                ...
                <ul class="rating-level">
                    <li><a class="one-star" data-star-value="{{items.value1}}" href="#">{{items.value1}}</a></li>
                    <li><a class="two-stars" data-star-value="{{items.value2}}" href="#">{{items.value2}}</a></li>
                    <li><a class="three-stars" data-star-value="{{items.value3}}" href="#">{{items.value3}}</a></li>
                    <li><a class="four-stars" data-star-value="{{items.value4}}" href="#">{{items.value4}}</a></li>
                    <li><a class="five-stars" data-star-value="{{items.value5}}" href="#">{{items.value5}}</a></li>
                </ul>
        ...
        _init: function() {
            ...
            S.each(self.stars, function(t, i, arr){
                ...
                S.each(DOM.query('a', t), function(a, j, arr) {
                    // construct default reason if undefined
                    if (self.config.reason[i][j] === undefined ) self.config.reason[i][j] = DEFAULT_REASON;
                    // construct default level if undefined
                    if (self.config.level[j] === undefined ) self.config.level[j] = DEFAULT_LEVEL;
                    ...
                    E.on(a, 'click', function(e){
                        // adjust current
                        DOM.removeClass(DOM.query(MA_RATING_CURRENT, t), MA_RATING_CURRENT);
                        DOM.addClass(a, MA_RATING_CURRENT);
                        // hide tips
                    ...
                    E.on(a, 'mouseover', function(e){
                        var offset = DOM.offset(this),
                            coffset = DOM.offset(self.rating),
                            h = DOM.height(this),
                            w = DOM.width(this);
                        // adjust poptips' location & msg
                        DOM.html(self.poptips, '<span><em>'+sc+'</em> 分 '+self.config.level[j]+'</span><strong>'+rs+'</strong>');
                        DOM.css(self.poptips, 'left', offset.left+w-coffset.left-100+'px'); // 100 is related with the width of self.poptips
                        DOM.css(self.poptips, 'top', offset.top-coffset.top+h+'px');
                        DOM.removeClass(self.poptips, HIDE);
                    });
                    
                    E.on(a, 'mouseout', function(e){
